...
public:
	Onslaught() {
		...
		prd_HumanControl				= new ControlPeriod::HumanControl(
			...
			rbt_Drivetrain, rbt_BallManagement, rbt_Gear, rbt_Climber, led_GearLight
		);
		...
		dsh_DriveStraight_P				= new MOLib::Dashboard::Number("[DriveStraight] P", 0.04);
		...
		dsh_DriveVision_P				= new MOLib::Dashboard::Number("[DriveVision] P", 0.01);
		...
		Jumper							= new WPILib::DigitalInput(8);
	...
	void RobotInit() {
		...
		enc_R_DriveDistance->SetReverseDirection(false);
		...
		rbt_Drivetrain->pid_DriveStraight->SetPID(0.01, 0.0, 0.0);
		...
		rbt_Drivetrain->pid_GoalAngle->SetOutputRange(-0.25, 0.25);
		...
		enc_B_ShooterSpeed->SetReverseDirection(false);
		...
		server->SetSize(CameraServer::kSize320x240);
